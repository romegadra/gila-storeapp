...
- Purchases are fake payments: the backend validates stock, records the purchase, and decrements inventory in one transaction.
...
- `POST /api/products/import-jobs` queues an asynchronous CSV import using multipart field `file`.
- `GET /api/products/import-jobs/{id}` returns async import status.
- `POST /api/purchases` creates a fake purchase.
